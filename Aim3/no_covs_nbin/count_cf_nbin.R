...
# x4 <- as.numeric(unique(data[ ,c("SubjectID","Gender")])[,2])
...
for(i in 1:nm){
  if(i == 1){
    fit1 <- glm(mediator[i,] ~ factor(trt) + factor(tvcon[i,]),
                family=binomial(link = "logit"))
  } else {
    fit1 <- glm(mediator[i,] ~ factor(trt) + factor(tvcon[i,]) + factor(mediator[i-1,]), 
                family=binomial(link = "logit"))
    m <- append(m, fit1$coefficients[[4]])
  }
...
for(i in 1:nm){
  if(i == 1){
    ...
    # start.coeff1 <- logbin(factor(outcome[i,]==0) ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]),
    ...
    # fit2 <- zeroinfl(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) | factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]),
    ...
    fit2 <- zeroinfl(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) | factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]),
                     na.action=na.omit, method = "Nelder-Mead",  link = "log",
                     start = list(count = c(log(start.c.p), rep(0, 3)), 
                                  zero = c(log(start.p[i]), rep(0, 3))))
  } else {
    # start.coeff <- coef(glm(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + outcome[i-1,] ,
    #                         family = poisson(link = "log"),
    #                         na.action=na.omit))
    ...
    fit2 <- zeroinfl(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) | factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + outcome[i-1,],
                     na.action=na.omit, dist = "negbin", method = "Nelder-Mead", link = "log",
                     start = list(count = c(log(start.c.p), rep(0, 3)), 
                                  zero = c(log(start.p[i]), rep(0, 4))))
    
    # o_count <- append(o_count, fit2$coefficients$count[[5]])
    o_zero <- append(o_zero, fit2$coefficients$zero[[5]])
  }
...
# results <- get(load("~/Aim3/no_covs_bin/results_varen_medf_count.rda"))
results <- get(load("~/Aim3/no_covs_nbin/results_cNRT_medf_count.rda"))
# 
# results <- get(load("~/Aim3/no_covs_bin/results_varen_medp_count.rda"))
# results <- get(load("~/Aim3/no_covs_bin/results_cNRT_medp_count.rda"))

# cov_avg <- mean(x1, na.rm = TRUE)
...
est_14 <- vector(length = length(est_2)) # this is just to maintain the current numbering

est_15 <- vector(length = length(est_2))
...
est_21 <- vector(length = length(est_2))
est_22 <- vector(length = length(est_2))
...
for (i in 1:length(est_2)) {
  est_15[i] <- (exp(est_2[i])/(1+exp(est_2[i])))*exp(est_7[i])
  
  if(i == 1){
    ### Overall Indirect Effect ###
    ## At baseline L(t+1)=1; C=cov_avg
    orNIE_l1 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_1[i] + est_3[i])))/((1 + exp(est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_3[i])))
    
    ## At baseline L(t+1)=0; C=cov_avg
    orNIE_l0 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_1[i])))/((1 + exp(est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i])))
    ...
    ## At baseline L(t+1)=1; C=cov_avg
    rrIES_l1 <- ((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_1[i] + est_3[i])))/((1 + exp(est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_3[i])))
    
    ## At baseline L(t+1)=0; C=cov_avg
    rrIES_l0 <- ((1 + exp(est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_1[i])))/((1 + exp(est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i])))
    ...
    ## At baseline L(t+1)=1; C=cov_avg
    rrIENS_l1 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_1[i] + est_3[i])))/((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_1[i] + est_3[i])))
    
    ## At baseline L(t+1)=0; C=cov_avg
    rrIENS_l0 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_1[i])))/((1 + exp(est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_1[i])))
    ...
    orNIE_m1l1 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_1[i] + est_3[i] + est_4[i-1])))/((1 + exp(est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_3[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
    orNIE_m0l1 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_1[i] + est_3[i])))/((1 + exp(est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_3[i])))
    
    ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
    orNIE_m1l0 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_1[i] + est_4[i-1])))/((1 + exp(est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
    orNIE_m0l0 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_1[i])))/((1 + exp(est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i])))
    ...
    ### Indirect Effect through susceptibility latent variable ###
    ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
    rrIES_m1l1 <- ((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_1[i] + est_3[i] + est_4[i-1])))/((1 + exp(est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_3[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
    rrIES_m0l1 <- ((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_1[i] + est_3[i])))/((1 + exp(est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_3[i])))
    
    ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
    rrIES_m1l0 <- ((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_1[i] + est_4[i-1])))/((1 + exp(est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_11[i] + est_1[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
    rrIES_m0l0 <- ((1 + exp(est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_1[i])))/((1 + exp(est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_11[i] + est_1[i])))
    ...
    ### Indirect Effect not through susceptibility latent variable ###
    ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
    rrIENS_m1l1 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_1[i] + est_3[i] + est_4[i-1])))/((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_1[i] + est_3[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
    rrIENS_m0l1 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_1[i] + est_3[i])))/((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_1[i] + est_3[i])))
    
    ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
    rrIENS_m1l0 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_1[i] + est_4[i-1])))/((1 + exp(est_11[i] + est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_1[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
    rrIENS_m0l0 <- ((1 + exp(est_7[i] + est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_1[i])))/((1 + exp(est_11[i] + est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_1[i])))
    
    ## IE at M(t)=1; L(t+1)=1; C=cov_avg
    est_24[i] <- rrIENS_m1l1
    
    ## IE at M(t)=0; L(t+1)=1; C=cov_avg
    est_25[i] <- rrIENS_m0l1
    
    ## IE at M(t)=1; L(t+1)=0; C=cov_avg
    est_26[i] <- rrIENS_m1l0
    
    ## IE at M(t)=0; L(t+1)=0; C=cov_avg
    est_27[i] <- rrIENS_m0l0
  }
}
...
est_6 <- predict(loess(effect_6 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_15 <- predict(loess(effect_15 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
...
est_21 <- predict(loess(effect_21 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_22 <- predict(loess(effect_22 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_23 <- predict(loess(effect_23 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
...
for (r in 1:R) {
  ...
  for(i in 1:nm){
    if(i == 1){
      ...
      fit1 <- glm(factor(mediator[i,index1]) ~ factor(trt[index1]) + factor(tvcon[i,index1]) + factor(mediator[i-1,index1]), 
                  family=binomial(link = "logit"))
      m <- append(m, fit1$coefficients[[4]])
    }
    a0 <- append(a0, fit1$coefficients[[1]])
  ...
  for(i in 1:nm){
    if(i == 1){
      ...
      # start.coeff1 <- logbin(factor(outcome[i,index1]==0) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]),
      #                        # family = binomial(link = "log"),
      #                        start = c(log(start.p[i]), rep(0,4)), maxit = 25000,
      #                        method = "em",
      #                        na.action=na.omit)
      ...
      # fit2 <- zeroinfl(outcome[i,index1] ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) | factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]),
      #                  na.action=na.omit, method = "Nelder-Mead",  link = "log",
      #                  start = list(count = c(log(start.c.p), rep(0, 4)), 
      #                               zero = c(log(start.z.p), rep(0, 4))))
      ...
      fit2 <- zeroinfl(outcome[i,index1] ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) | factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + outcome[i-1,index1],
                       na.action=na.omit, dist = "negbin", method = "Nelder-Mead", link = "log",
                       start = list(count = c(log(start.c.p), rep(0, 3)), 
                                    zero = c(log(start.p[i]), rep(0, 4))))
  ...
  for(i in 1:nm){
    if(i == 1){
      ### Overall Indirect Effect ###
      ## At baseline L(t+1)=1; C=cov_avg
      orNIE_l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(a0[i] + lm[i])))/((1 + exp(a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i])))
      
      ## At baseline L(t+1)=0; C=cov_avg
      orNIE_l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i]))*(1 + exp(a0[i])))/((1 + exp(a0[i] + a1[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i])))
      ...
      ### Indirect Effect through susceptibility latent variable ###
      ## At baseline L(t+1)=1; C=cov_avg
      rrIES_l1 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + a0[i] + lm[i])))/((1 + exp(a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i])))
      
      ## At baseline L(t+1)=0; C=cov_avg
      rrIES_l0 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i]))*(1 + exp(b1_count[i] + a0[i])))/((1 + exp(a0[i] + a1[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i])))
      ...
      ### Indirect Effect not through susceptibility latent variable ###
      ## At baseline L(t+1)=1; C=cov_avg
      rrIENS_l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(a0[i] + lm[i])))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + a0[i] + lm[i])))
      
      ## At baseline L(t+1)=0; C=cov_avg
      rrIENS_l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i]))*(1 + exp(a0[i])))/((1 + exp(b1_zero[i] + a0[i] + a1[i]))*(1 + exp(b1_count[i] + a0[i])))
      ...
      ### Overall Indirect Effect ###
      ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
      orNIE_m1l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(a0[i] + lm[i] + m[i-1])))/((1 + exp(a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
      orNIE_m0l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(a0[i] + lm[i])))/((1 + exp(a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i])))
      
      ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
      orNIE_m1l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + m[i-1]))*(1 + exp(a0[i] + m[i-1])))/((1 + exp(a0[i] + a1[i] + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
      orNIE_m0l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i]))*(1 + exp(a0[i])))/((1 + exp(a0[i] + a1[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i])))
      ...
      ### Indirect Effect through susceptibility latent variable ###
      ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
      rrIES_m1l1 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + lm[i] + m[i-1])))/((1 + exp(a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
      rrIES_m0l1 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + a0[i] + lm[i])))/((1 + exp(a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i])))
      
      ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
      rrIES_m1l0 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + m[i-1])))/((1 + exp(a0[i] + a1[i] + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
      rrIES_m0l0 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i]))*(1 + exp(b1_count[i] + a0[i])))/((1 + exp(a0[i] + a1[i]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i])))
      ...
      ### Indirect Effect not through susceptibility latent variable ###
      ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
      rrIENS_m1l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(a0[i] + lm[i] + m[i-1])))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + lm[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
      rrIENS_m0l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(a0[i] + lm[i])))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(b1_count[i] + a0[i] + lm[i])))
      
      ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
      rrIENS_m1l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + m[i-1]))*(1 + exp(a0[i] + m[i-1])))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
      rrIENS_m0l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i]))*(1 + exp(a0[i])))/((1 + exp(b1_zero[i] + a0[i] + a1[i]))*(1 + exp(b1_count[i] + a0[i])))
...
# save(CI, file = paste("CI_varen_medf_count.rda"))
save(CI, file = paste("CI_cNRT_medf_count.rda"))
# 
# save(CI, file = paste("CI_varen_medp_count.rda"))
# save(CI, file = paste("CI_cNRT_medp_count.rda"))
