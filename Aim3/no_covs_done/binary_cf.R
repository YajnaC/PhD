...
for(i in 1:nm){
  if(i == 1){
    # start.coeff <- coef(glm(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]),
    #                         family = poisson(link = "log"),
    #                         na.action=na.omit))
    fit2 <- logbin(factor(outcome[i,]) ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]), 
                   start = c(log(start.p[i]), rep(0,3)), maxit = 25000,
                   method = "em",
                   na.action=na.omit)
  } else {
    ...
    #                         family = poisson(link = "log"),
    #                         na.action=na.omit))
    fit2 <- logbin(factor(outcome[i,]) ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + factor(outcome[i-1,]), 
                   start = c(log(start.p[i]), rep(0,4)), maxit = 25000,
                   method = "em",
                   na.action=na.omit)
    o <- append(o, fit2$coefficients[[5]])
  }
...
# results <- get(load("~/Aim3/no_covs/results_varen_medf_binary.rda"))
results <- get(load("~/Aim3/no_covs/results_cNRT_medf_binary.rda"))
# 
# results <- get(load("~/Aim3/no_covs/results_varen_medp_binary.rda"))
# results <- get(load("~/Aim3/no_covs/results_cNRT_medp_binary.rda"))
...
est_10 <- vector(length = length(est_2))
est_11 <- vector(length = length(est_2))
est_12 <- vector(length = length(est_2))
est_13 <- vector(length = length(est_2))
est_14 <- vector(length = length(est_2))

for (i in 1:length(est_2)) {
  est_10[i] <- (exp(est_2[i])/(1+exp(est_2[i])))*exp(est_7[i])
  
  if(i == 1){
    ## At baseline L(t+1)=1; C=avg cov
    orNIE_l1 <- ((1 + exp(est_7[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_1[i] + est_3[i])))/((1 + exp(est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_1[i] + est_3[i])))
    
    ## At baseline L(t+1)=0; C=avg cov
    orNIE_l0 <- ((1 + exp(est_7[i] + est_1[i] + est_2[i]))*(1 + exp(est_1[i])))/((1 + exp(est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_1[i])))
    ...
    orNIE_m1l1 <- ((1 + exp(est_7[i] + est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_1[i] + est_3[i] + est_4[i-1])))/((1 + exp(est_1[i] + est_2[i] + est_3[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_1[i] + est_3[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=1; C=avg cov
    orNIE_m0l1 <- ((1 + exp(est_7[i] + est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_1[i] + est_3[i])))/((1 + exp(est_1[i] + est_2[i] + est_3[i]))*(1 + exp(est_7[i] + est_1[i] + est_3[i])))
    
    ## At t+1 M(t)=1; L(t+1)=0; C=avg cov
    orNIE_m1l0 <- ((1 + exp(est_7[i] + est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_1[i] + est_4[i-1])))/((1 + exp(est_1[i] + est_2[i] + est_4[i-1]))*(1 + exp(est_7[i] + est_1[i] + est_4[i-1])))
    
    ## At t+1 M(t)=0; L(t+1)=0; C=avg cov
    orNIE_m0l0 <- ((1 + exp(est_7[i] + est_1[i] + est_2[i]))*(1 + exp(est_1[i])))/((1 + exp(est_1[i] + est_2[i]))*(1 + exp(est_7[i] + est_1[i])))
...
est_10 <- predict(loess(effect_10 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_11 <- predict(loess(effect_11 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_12 <- predict(loess(effect_12 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_13 <- predict(loess(effect_13 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
est_14 <- predict(loess(effect_14 ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))

est_smooth <- vector("list", 14)
...
for (r in 1:R) {
  ...
  for(i in 1:nm){
    if(i == 1){
      fit2 <- logbin(factor(outcome[i,index1]) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]), 
                     start = c(log(start.p[i]), rep(c(0), 3)),
                     method = "em", maxit = 25000,
                     na.action=na.omit)
    } else {
      fit2 <- logbin(factor(outcome[i,index1]) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + factor(outcome[i-1,index1]), 
                     start = c(log(start.p[i]), rep(c(0), 4)),
                     method = "em", maxit = 25000,
                     na.action=na.omit)
      o <- append(o, fit2$coefficients[[5]])
    }
  ...
  for(i in 1:nm){
    if(i == 1){
      ...
      orNIE_l1 <- ((1 + exp(b1[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(a0[i] + lm[i])))/((1 + exp(a0[i] + a1[i] + lm[i]))*(1 + exp(b1[i] + a0[i] + lm[i])))
      
      ## At baseline L(t+1)=0; C=cov_avg
      orNIE_l0 <- ((1 + exp(b1[i] + a0[i] + a1[i]))*(1 + exp(a0[i])))/((1 + exp(a0[i] + a1[i]))*(1 + exp(b1[i] + a0[i])))
      ...
      orNIE_m1l1 <- ((1 + exp(b1[i] + a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(a0[i] + lm[i] + m[i-1])))/((1 + exp(a0[i] + a1[i] + lm[i] + m[i-1]))*(1 + exp(b1[i] + a0[i] + lm[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
      orNIE_m0l1 <- ((1 + exp(b1[i] + a0[i] + a1[i] + lm[i]))*(1 + exp(a0[i] + lm[i])))/((1 + exp(a0[i] + a1[i] + lm[i]))*(1 + exp(b1[i] + a0[i] + lm[i])))
      
      ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
      orNIE_m1l0 <- ((1 + exp(b1[i] + a0[i] + a1[i] + m[i-1]))*(1 + exp(a0[i] + m[i-1])))/((1 + exp(a0[i] + a1[i] + m[i-1]))*(1 + exp(b1[i] + a0[i] + m[i-1])))
      
      ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
      orNIE_m0l0 <- ((1 + exp(b1[i] + a0[i] + a1[i]))*(1 + exp(a0[i])))/((1 + exp(a0[i] + a1[i]))*(1 + exp(b1[i] + a0[i])))
...
# save(CI, file = paste("CI_varen_medf_binary.rda"))
save(CI, file = paste("CI_cNRT_medf_binary.rda"))
# 
# save(CI, file = paste("CI_varen_medp_binary.rda"))
# save(CI, file = paste("CI_cNRT_medp_binary.rda"))
