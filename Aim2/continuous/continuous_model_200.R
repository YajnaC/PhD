...
for (r in 1:R) {
  ...
  outcome <- LongToWide(aim2$id, aim2$time_pt, aim2$Ycrav)
  ...
  for(i in 1:nm){
    ...
    # x_2m <- append(x_2m, fit1$coefficients[[4]])
  }
  ...
  for(i in 1:nm){
    if(i == 1){
      fit2 <- lm(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1, na.action=na.omit)
    } else {
      fit2 <- lm(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1 + outcome[i-1,], na.action=na.omit)
...
save(results_continuous, file = paste("results_cont_200_dat.rda", sep = ""))
