### Although the file name says binary_c_bootstrap.R this is in fact for the count non-zero inflated model ###
...
# setwd("~/Aim1/binary_c500")

# results <- get(load("~/Aim1/binary_c500/results_count_nzi_500.rda"))
results <- get(load("~/Aim1/count_nzi/results_count_nzi_500.rda"))
...
# k = 1
  ...
  for (r in 1:R) {
    ...
    outcome <- LongToWide(coeff_data$id, coeff_data$time_pt, coeff_data$Cigr)
    ...
    for(i in 1:nm){
      if(i == 1){
        fit2 <- glm(outcome[i,index1] ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1], 
                                                family=poisson(link = "log"))
      } else {
        fit2 <- glm(outcome[i,index1] ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1] + outcome[i-1,index1], 
                                                family=poisson(link = "log"))
  ...
  for (q in 1:nrow(mat_a0)) {
    ...
    for(i in 1:20){
      if(i == 1){
        ...
        orNIE_l1 <- ((1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_a1[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_a0[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg)))/((1 + exp(mat_a0[q,i] + mat_a1[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg)))
        
        ## At baseline L(t+1)=0; C=cov_avg
        orNIE_l0 <- ((1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_a1[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_a0[q,i] + mat_x1m[q,i]*cov_avg)))/((1 + exp(mat_a0[q,i] + mat_a1[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_x1m[q,i]*cov_avg)))
        ...
        orNIE_m1l1 <- ((1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_a1[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1]))*(1 + exp(mat_a0[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1])))/((1 + exp(mat_a0[q,i] + mat_a1[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1]))*(1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
        orNIE_m0l1 <- ((1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_a1[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_a0[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg)))/((1 + exp(mat_a0[q,i] + mat_a1[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_lm[q,i] + mat_x1m[q,i]*cov_avg)))
        
        ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
        orNIE_m1l0 <- ((1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_a1[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1]))*(1 + exp(mat_a0[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1])))/((1 + exp(mat_a0[q,i] + mat_a1[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1]))*(1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_x1m[q,i]*cov_avg + mat_m[q,i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
        orNIE_m0l0 <- ((1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_a1[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_a0[q,i] + mat_x1m[q,i]*cov_avg)))/((1 + exp(mat_a0[q,i] + mat_a1[q,i] + mat_x1m[q,i]*cov_avg))*(1 + exp(mat_b1[q,i] + mat_a0[q,i] + mat_x1m[q,i]*cov_avg)))
  ...
  save(CI, file = paste("CI_count_nzi_500_",k,".rda", sep = ""))
