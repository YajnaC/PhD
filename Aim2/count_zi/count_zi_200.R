...
mean.y <- as.data.frame(table(aim2_dat$time_pt, aim2_dat$Cigc1)) %>%
  filter(Var2 == 0)
...
for (r in 1:R) {
  ...
  outcome <- LongToWide(aim2$id, aim2$time_pt, aim2$Cigc1)
  ...
  for(i in 1:nm){
    if(i == 1){
      fit2 <- zeroinfl(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1 | factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1,
                       ...
                       start = list(count = c(rep(c(0), 5)), zero = c(log(start.p[i]), rep(c(0), 4))))
    } else {
      fit2 <- zeroinfl(outcome[i,] ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1 + outcome[i-1,] | factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1 + outcome[i-1,],
                       na.action=na.omit, method = "Nelder-Mead", link = "log",
                       start = list(count = c(rep(c(0), 6)), zero = c(log(start.p[i]), rep(c(0), 5))))
...
save(results_continuous, file = paste("results_count_zi_200.rda", sep = ""))
