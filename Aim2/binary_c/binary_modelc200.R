...
mean.y <- as.data.frame(table(aim2_dat$time_pt, aim2_dat$Yc)) %>%
  filter(Var2 == 1)
...
for (r in 1:R) {
  ...
  outcome <- LongToWide(aim2$id, aim2$time_pt, aim2$Yc)
...
save(results_continuous, file = paste("results_binary_200_modelc.rda", sep = ""))
