### Models for binary outcome - rare ###
...
for (r in 1:R) {
  ...
  outcome <- LongToWide(aim2$id, aim2$time_pt, aim2$Yr)
  ...
  for(i in 1:nm){
    if(i == 1){
      fit2 <- glm(factor(outcome[i,]) ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1,
                  family=binomial(link = "logit"), na.action=na.omit)
    } else {
      fit2 <- glm(factor(outcome[i,]) ~ factor(trt) + factor(mediator[i,]) + factor(tvcon[i,]) + x1 + factor(outcome[i-1,]),
                  family=binomial(link = "logit"), na.action=na.omit)
...
save(results_continuous, file = paste("results_binary_200_modelr.rda", sep = ""))
