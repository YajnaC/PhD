...
results <- get(load("~/Aim1/binary_r/results_binary_500_modelr.rda"))
...
for (k in 1:nrow(results$mat_id)) {
  ...
  for (r in 1:R) {
    ...
    outcome <- LongToWide(coeff_data$id, coeff_data$time_pt, coeff_data$Yr)
    ...
    for(i in 1:nm){
      if(i == 1){
        fit2 <- glm(factor(outcome[i,index1]) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1],
                    family=binomial(link = "logit"))
      } else {
        fit2 <- glm(factor(outcome[i,index1]) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1] + factor(outcome[i-1,index1]),
                    family=binomial(link = "logit"))
...
save(CI, file = paste("CI_binary_500_rare.rda", sep = ""))
