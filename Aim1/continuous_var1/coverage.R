...
for (k in 1:2000) {
  CI_part <- get(load(paste("~/Aim1/continuous_var1/CI_cont_500_",k,".rda", sep = "")))
  for(z in 1:5){
    CI_full[[z]][[k]] <- CI_part[[z]][[k]] 
  }
}

save(CI_full, file = paste("CI_cont_500_full.rda", sep = ""))

CI <- get(load("~/Aim1/continuous_var1/CI_cont_500_full.rda"))
...
id_med = 727
# id_med = 1202
### Sample ID 1202 rendered a product of coefficients estimate closest to the median ###
