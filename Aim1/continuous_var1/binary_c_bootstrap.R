...
setwd("~/Aim1/continuous_var1")

results <- get(load("~/Aim1/continuous_var1/results_cont_500_dat.rda"))
...
# for (k in 1:nrow(results$mat_id)) {
  ...
  for (r in 1:R) {
    ...
    # # mat_x2o[r,] <- x_2o
    # mat_o[r,] <- o
    mat_ab[r,] <- predict(loess((exp(a1)*b1) ~ t.seq[1:nm], span = 0.5, degree = 1))
    ...
    for(i in 1:nm){
      if(i == 1){
        ...
        prob1_l1 <- exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))
        prob0_l1 <- exp(a0[i] + lm[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg))
        
        ## At baseline L(t+1)=0; C=cov_avg
        prob1_l0 <- exp(a0[i] + a1[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg))
        prob0_l0 <- exp(a0[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + x_1m[i]*cov_avg))
        
        ## IE at M(t)=1; L(t+1)=1; C=cov_avg
        IE11 <- append(IE11, (b1[i]*(prob1_l1 - prob0_l1)))
        
        ## IE at M(t)=0; L(t+1)=1; C=cov_avg
        IE01 <- append(IE01, (b1[i]*(prob1_l1 - prob0_l1)))
        
        ## IE at M(t)=1; L(t+1)=0; C=cov_avg
        IE10 <- append(IE10, (b1[i]*(prob1_l0 - prob0_l0)))
        
        ## IE at M(t)=0; L(t+1)=0; C=cov_avg
        IE00 <- append(IE00, (b1[i]*(prob1_l0 - prob0_l0)))
        
      }else{
        ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
        prob1_m1l1 <- exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])/(1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))
        prob0_m1l1 <- exp(a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])/(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))
        
        ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
        prob1_m0l1 <- exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))
        prob0_m0l1 <- exp(a0[i] + lm[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg))
        
        ## At t+1 M(t)1; L(t+1)=0; C=cov_avg
        prob1_m1l0 <- exp(a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1])/(1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))
        prob0_m1l0 <- exp(a0[i] + x_1m[i]*cov_avg + m[i-1])/(1 + exp(a0[i] + x_1m[i]*cov_avg + m[i-1]))
        
        ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
        prob1_m0l0 <- exp(a0[i] + a1[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg))
        prob0_m0l0 <- exp(a0[i] + x_1m[i]*cov_avg)/(1 + exp(a0[i] + x_1m[i]*cov_avg))
        
        ## IE at M(t)=1; L(t+1)=1; C=cov_avg
        IE11 <- append(IE11, (b1[i]*(prob1_m1l1 - prob0_m1l1)))
        
        ## IE at M(t)=0; L(t+1)=1; C=cov_avg
        IE01 <- append(IE01, (b1[i]*(prob1_m0l1 - prob0_m0l1)))
        
        ## IE at M(t)=1; L(t+1)=0; C=cov_avg
        IE10 <- append(IE10, (b1[i]*(prob1_m1l0 - prob0_m1l0)))
        
        ## IE at M(t)=0; L(t+1)=0; C=cov_avg
        IE00 <- append(IE00, (b1[i]*(prob1_m0l0 - prob0_m0l0)))
      }
    }
  ...
  # effect_interest[[12]] <- predict(loess(exp(mat_a1)*mat_b1~t.seq[1:nm], span = 0.5, degree = 1))
  # 
...
#     if(i == 1){
#       ## At baseline L(t+1)=1; C=cov_avg
#       prob1_l1 <- exp(mat_a0[i] + mat_a1[i] + mat_lm[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_a1[i] + mat_lm[i] + mat_x1m[i]*cov_avg))
#       prob0_l1 <- exp(mat_a0[i] + mat_lm[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_lm[i] + mat_x1m[i]*cov_avg))
#       
#       ## At baseline L(t+1)=0; C=cov_avg
#       prob1_l0 <- exp(mat_a0[i] + mat_a1[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_a1[i] + mat_x1m[i]*cov_avg))
#       prob0_l0 <- exp(mat_a0[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_x1m[i]*cov_avg))
#       
#       ## IE at M(t)=1; L(t+1)=1; C=cov_avg
#       IE11 <- append(IE11, (mat_b1[i]*(prob1_l1 - prob0_l1)))
#       
#       ## IE at M(t)=0; L(t+1)=1; C=cov_avg
#       IE01 <- append(IE01, (mat_b1[i]*(prob1_l1 - prob0_l1)))
#       
#       ## IE at M(t)=1; L(t+1)=0; C=cov_avg
#       IE10 <- append(IE10, (mat_b1[i]*(prob1_l0 - prob0_l0)))
#       
#       ## IE at M(t)=0; L(t+1)=0; C=cov_avg
#       IE00 <- append(IE00, (mat_b1[i]*(prob1_l0 - prob0_l0)))
#       
#     }else{
#       ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
#       prob1_m1l1 <- exp(mat_a0[i] + mat_a1[i] + mat_lm[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1])/(1 + exp(mat_a0[i] + mat_a1[i] + mat_lm[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1]))
#       prob0_m1l1 <- exp(mat_a0[i] + mat_lm[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1])/(1 + exp(mat_a0[i] + mat_lm[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1]))
#       
#       ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
#       prob1_m0l1 <- exp(mat_a0[i] + mat_a1[i] + mat_lm[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_a1[i] + mat_lm[i] + mat_x1m[i]*cov_avg))
#       prob0_m0l1 <- exp(mat_a0[i] + mat_lm[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_lm[i] + mat_x1m[i]*cov_avg))
#       
#       ## At t+1 M(t)1; L(t+1)=0; C=cov_avg
#       prob1_m1l0 <- exp(mat_a0[i] + mat_a1[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1])/(1 + exp(mat_a0[i] + mat_a1[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1]))
#       prob0_m1l0 <- exp(mat_a0[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1])/(1 + exp(mat_a0[i] + mat_x1m[i]*cov_avg + mat_m[q,i-1]))
#       
#       ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
#       prob1_m0l0 <- exp(mat_a0[i] + mat_a1[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_a1[i] + mat_x1m[i]*cov_avg))
#       prob0_m0l0 <- exp(mat_a0[i] + mat_x1m[i]*cov_avg)/(1 + exp(mat_a0[i] + mat_x1m[i]*cov_avg))
#       
#       ## IE at M(t)=1; L(t+1)=1; C=cov_avg
#       IE11 <- append(IE11, (mat_b1[i]*(prob1_m1l1 - prob0_m1l1)))
#       
#       ## IE at M(t)=0; L(t+1)=1; C=cov_avg
#       IE01 <- append(IE01, (mat_b1[i]*(prob1_m0l1 - prob0_m0l1)))
#       
#       ## IE at M(t)=1; L(t+1)=0; C=cov_avg
#       IE10 <- append(IE10, (mat_b1[i]*(prob1_m1l0 - prob0_m1l0)))
#       
#       ## IE at M(t)=0; L(t+1)=0; C=cov_avg
#       IE00 <- append(IE00, (mat_b1[i]*(prob1_m0l0 - prob0_m0l0)))
#     }
#   }
...
for(z in 1:length(effect_interest)){
  ...
  save(CI, file = paste("CI_cont_500_",k,".rda", sep = ""))
# }
