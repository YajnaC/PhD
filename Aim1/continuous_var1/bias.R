### Bias analysis for continuous outcome ###
...
# results <- get(load("~/Aim1/continuous_var1/results_cont_200_dat.rda"))
results <- get(load("~/Aim1/continuous_var1/results_cont_500_dat.rda"))
# results <- get(load("~/Aim1/continuous_var1/results_cont_700_dat.rda"))
...
for (i in 1:ncol(est_2)) {
  est_12[,i] <- est_8[,i]*exp(est_2[,i])
  
  if(i == 1){
    ## At baseline L(t+1)=1; C=avg cov
    pr1_l1 <- exp(est_1[,i] + est_2[,i] + est_3[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_2[,i] + est_3[,i] + est_4[,i]*cov_avg))
    pr0_l1 <- exp(est_1[,i] + est_3[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_3[,i] + est_4[,i]*cov_avg))
    
    ## At baseline L(t+1)=0; C=avg cov
    pr1_l0 <- exp(est_1[,i] + est_2[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_2[,i] + est_4[,i]*cov_avg))
    pr0_l0 <- exp(est_1[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_4[,i]*cov_avg))
    
    ## IE at M(t)=1; L(t+1)=1; C=avg cov
    est_13[,i] <- est_8[,i]*(pr1_l1 - pr0_l1)
    
    ## IE at M(t)=0; L(t+1)=1; C=avg cov
    est_14[,i] <- est_8[,i]*(pr1_l1 - pr0_l1)
    
    ## IE at M(t)=1; L(t+1)=0; C=avg cov
    est_15[,i] <- est_8[,i]*(pr1_l0 - pr0_l0)
    
    ## IE at M(t)=0; L(t+1)=0; C=avg cov
    est_16[,i] <- est_8[,i]*(pr1_l0 - pr0_l0)
    
  }else{
    ## At t+1 M(t)=1; L(t+1)=1; C=avg cov
    pr1_m1l1 <- exp(est_1[,i] + est_2[,i] + est_3[,i] + est_4[,i]*cov_avg + est_5[,i-1])/(1 + exp(est_1[,i] + est_2[,i] + est_3[,i] + est_4[,i]*cov_avg + est_5[,i-1]))
    pr0_m1l1 <- exp(est_1[,i] + est_3[,i] + est_4[,i]*cov_avg + est_5[,i-1])/(1 + exp(est_1[,i] + est_3[,i] + est_4[,i]*cov_avg + est_5[,i-1]))
    
    ## At t+1 M(t)=0; L(t+1)=1; C=avg cov
    pr1_m0l1 <- exp(est_1[,i] + est_2[,i] + est_3[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_2[,i] + est_3[,i] + est_4[,i]*cov_avg))
    pr0_m0l1 <- exp(est_1[,i] + est_3[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_3[,i] + est_4[,i]*cov_avg))
    
    ## At t+1 M(t)1; L(t+1)=0; C=avg cov
    pr1_m1l0 <- exp(est_1[,i] + est_2[,i] + est_4[,i]*cov_avg + est_5[,i-1])/(1 + exp(est_1[,i] + est_2[,i] + est_4[,i]*cov_avg + est_5[,i-1]))
    pr0_m1l0 <- exp(est_1[,i] + est_4[,i]*cov_avg + est_5[,i-1])/(1 + exp(est_1[,i] + est_4[,i]*cov_avg + est_5[,i-1]))
    
    ## At t+1 M(t)=0; L(t+1)=0; C=avg cov
    pr1_m0l0 <- exp(est_1[,i] + est_2[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_2[,i] + est_4[,i]*cov_avg))
    pr0_m0l0 <- exp(est_1[,i] + est_4[,i]*cov_avg)/(1 + exp(est_1[,i] + est_4[,i]*cov_avg))
    
    ## IE at M(t)=1; L(t+1)=1; C=avg cov
    est_13[,i] <- est_8[,i]*(pr1_m1l1 - pr0_m1l1)
    
    ## IE at M(t)=0; L(t+1)=1; C=avg cov
    est_14[,i] <- est_8[,i]*(pr1_m0l1 - pr0_m0l1)
    
    ## IE at M(t)=1; L(t+1)=0; C=avg cov
    est_15[,i] <- est_8[,i]*(pr1_m1l0 - pr0_m1l0)
    
    ## IE at M(t)=0; L(t+1)=0; C=avg cov
    est_16[,i] <- est_8[,i]*(pr1_m0l0 - pr0_m0l0)
  }
}
