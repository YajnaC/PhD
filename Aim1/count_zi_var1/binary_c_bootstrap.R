...
setwd("~/Aim1/count_zi_var1")

results <- get(load("~/Aim1/count_zi_var1/results_count_zi_500_v2.rda"))
...
# k = 1
  ...
  mat_ab <- matrix(nrow = R, ncol = 20)
  mat_IE11 <- matrix(nrow = R, ncol = 20)
  mat_IE01 <- matrix(nrow = R, ncol = 20)
  mat_IE10 <- matrix(nrow = R, ncol = 20)
  mat_IE00 <- matrix(nrow = R, ncol = 20)
  
  mat_IES11 <- matrix(nrow = R, ncol = 20)
  mat_IES01 <- matrix(nrow = R, ncol = 20)
  mat_IES10 <- matrix(nrow = R, ncol = 20)
  mat_IES00 <- matrix(nrow = R, ncol = 20)
  
  mat_IENS11 <- matrix(nrow = R, ncol = 20)
  mat_IENS01 <- matrix(nrow = R, ncol = 20)
  mat_IENS10 <- matrix(nrow = R, ncol = 20)
  mat_IENS00 <- matrix(nrow = R, ncol = 20)
  ...
  for (r in 1:R) {
    ...
    for(i in 1:nm){
      if(i == 1){
        ...
        orNIE_l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))
        
        ## At baseline L(t+1)=0; C=cov_avg
        orNIE_l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + x_1m[i]*cov_avg)))
        ...
        ### Indirect Effect through susceptibility latent variable ###
        ## At baseline L(t+1)=1; C=cov_avg
        rrIES_l1 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))
        
        ## At baseline L(t+1)=0; C=cov_avg
        rrIES_l0 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + x_1m[i]*cov_avg)))
        ...
        ### Indirect Effect not through susceptibility latent variable ###
        ## At baseline L(t+1)=1; C=cov_avg
        rrIENS_l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg)))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))
        
        ## At baseline L(t+1)=0; C=cov_avg
        rrIENS_l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + x_1m[i]*cov_avg)))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + x_1m[i]*cov_avg)))
        ...
        orNIE_m1l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])))/((1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
        orNIE_m0l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))
        
        ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
        orNIE_m1l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(a0[i] + x_1m[i]*cov_avg + m[i-1])))/((1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + x_1m[i]*cov_avg + m[i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
        orNIE_m0l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + x_1m[i]*cov_avg)))
        ...
        ### Indirect Effect through susceptibility latent variable ###
        ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
        rrIES_m1l1 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])))/((1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
        rrIES_m0l1 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))
        
        ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
        rrIES_m1l0 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + x_1m[i]*cov_avg + m[i-1])))/((1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + x_1m[i]*cov_avg + m[i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
        rrIES_m0l0 <- ((1 + exp(b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + x_1m[i]*cov_avg)))/((1 + exp(a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + b1_zero[i] + a0[i] + x_1m[i]*cov_avg)))
        ...
        ### Indirect Effect not through susceptibility latent variable ###
        ## At t+1 M(t)=1; L(t+1)=1; C=cov_avg
        rrIENS_m1l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + lm[i] + x_1m[i]*cov_avg + m[i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=1; C=cov_avg
        rrIENS_m0l1 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + lm[i] + x_1m[i]*cov_avg)))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + lm[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + lm[i] + x_1m[i]*cov_avg)))
        
        ## At t+1 M(t)=1; L(t+1)=0; C=cov_avg
        rrIENS_m1l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(a0[i] + x_1m[i]*cov_avg + m[i-1])))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg + m[i-1]))*(1 + exp(b1_count[i] + a0[i] + x_1m[i]*cov_avg + m[i-1])))
        
        ## At t+1 M(t)=0; L(t+1)=0; C=cov_avg
        rrIENS_m0l0 <- ((1 + exp(b1_count[i] + b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(a0[i] + x_1m[i]*cov_avg)))/((1 + exp(b1_zero[i] + a0[i] + a1[i] + x_1m[i]*cov_avg))*(1 + exp(b1_count[i] + a0[i] + x_1m[i]*cov_avg)))
  ...
  effect_interest <- vector("list",13)
  
  effect_interest[[1]] <- mat_ab
  effect_interest[[2]] <- mat_IE11
  effect_interest[[3]] <- mat_IE01
  effect_interest[[4]] <- mat_IE10
  effect_interest[[5]] <- mat_IE00
  
  effect_interest[[6]] <- mat_IES11
  effect_interest[[7]] <- mat_IES01
  effect_interest[[8]] <- mat_IES10
  effect_interest[[9]] <- mat_IES00
  
  effect_interest[[10]] <- mat_IENS11
  effect_interest[[11]] <- mat_IENS01
  effect_interest[[12]] <- mat_IENS10
  effect_interest[[13]] <- mat_IENS00
