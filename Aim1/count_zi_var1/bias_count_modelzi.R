...
# results <- get(load("~/Aim1/count_zi_var1/results_count_zi_700_v2.rda"))
...
for(i in 1:nrow(effect_18)){
  est_18[i,] <- predict(loess(effect_18[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  
  est_19[i,] <- predict(loess(effect_19[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_20[i,] <- predict(loess(effect_20[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_21[i,] <- predict(loess(effect_21[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_22[i,] <- predict(loess(effect_22[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  
  est_23[i,] <- predict(loess(effect_23[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_24[i,] <- predict(loess(effect_24[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_25[i,] <- predict(loess(effect_25[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_26[i,] <- predict(loess(effect_26[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  
  est_27[i,] <- predict(loess(effect_27[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_28[i,] <- predict(loess(effect_28[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_29[i,] <- predict(loess(effect_29[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
  est_30[i,] <- predict(loess(effect_30[i,] ~ t.seq[1:length(t.seq)], span = 0.5, degree = 1))
}
...
bias_tv_made <- bias_tv_made[-c(1398, 898, 211, 915, 554, 579, 
                                1219, 1858, 476, 1794, 1152, 776, 
                                365, 669, 1276, 361, 1215, 1410),]
...
bias_tv_wase <- bias_tv_wase[-c(1398, 898, 211, 915, 554, 579, 
                                1219, 1858, 476, 1794, 1152, 776, 
                                365, 669, 1276, 361, 1215, 1410),]
...
tiv_bias$var <- factor(tiv_bias$variable,
                       levels = c("a0", "lm", "x1m", "m", "b0", "lo", "x1o", "o", "b0_zero", "g_zero", "b1_zero", "lo_zero", "x1o_zero", "o_zero"),
                       labels = c("α0", "α2", "α3'", "α4", "β0", "β2", "β3'", "β4", "β0'", "γ(t)'", "β1(t)'", "β2'", "β3''", "β4'"))

p5 <- ggplot(tiv_bias, aes(x=var, y=value)) +
