...
for (k in 1:2000) {
  CI_part <- get(load(paste("~/Aim1/count_zi_var1/CI_count_zi_500_",k,".rda", sep = "")))
  for(z in 1:13){
...
CI <- get(load("~/Aim1/count_zi_var1/CI_count_zi_500_full.rda"))
...
var_names <- c("α1(t)×β1(t)", "POF OIE11", "POF OIE01", "POF OIE10", "POF OIE00",
...
effect_interest[[1]] <- est_smooth[[18]]

effect_interest[[2]] <- est_smooth[[19]]
effect_interest[[3]] <- est_smooth[[20]]
effect_interest[[4]] <- est_smooth[[21]]
effect_interest[[5]] <- est_smooth[[22]]

effect_interest[[6]] <- est_smooth[[23]]
effect_interest[[7]] <- est_smooth[[24]]
effect_interest[[8]] <- est_smooth[[25]]
effect_interest[[9]] <- est_smooth[[26]]

effect_interest[[10]] <- est_smooth[[27]]
effect_interest[[11]] <- est_smooth[[28]]
effect_interest[[12]] <- est_smooth[[29]]
effect_interest[[13]] <- est_smooth[[30]]

t.est <- seq(from = 1, to = 20, length.out = 20)

# dat_g <- as.data.frame(cbind(effect_interest[[1]][id_med,], CI[[7]][[id_med]], t.est))
dat_ab <- as.data.frame(cbind(effect_interest[[1]][id_med,], CI[[1]][[id_med]], t.est))

dat_OIE11 <- as.data.frame(cbind(effect_interest[[2]][id_med,], CI[[2]][[id_med]]))
dat_OIE01 <- as.data.frame(cbind(effect_interest[[3]][id_med,], CI[[3]][[id_med]]))
dat_OIE10 <- as.data.frame(cbind(effect_interest[[4]][id_med,], CI[[4]][[id_med]]))
dat_OIE00 <- as.data.frame(cbind(effect_interest[[5]][id_med,], CI[[5]][[id_med]]))

dat_IES11 <- as.data.frame(cbind(effect_interest[[6]][id_med,], CI[[6]][[id_med]]))
dat_IES01 <- as.data.frame(cbind(effect_interest[[7]][id_med,], CI[[7]][[id_med]]))
dat_IES10 <- as.data.frame(cbind(effect_interest[[8]][id_med,], CI[[8]][[id_med]]))
dat_IES00 <- as.data.frame(cbind(effect_interest[[9]][id_med,], CI[[9]][[id_med]]))

dat_IENS11 <- as.data.frame(cbind(effect_interest[[10]][id_med,], CI[[10]][[id_med]]))
dat_IENS01 <- as.data.frame(cbind(effect_interest[[11]][id_med,], CI[[11]][[id_med]]))
dat_IENS10 <- as.data.frame(cbind(effect_interest[[12]][id_med,], CI[[12]][[id_med]]))
dat_IENS00 <- as.data.frame(cbind(effect_interest[[13]][id_med,], CI[[13]][[id_med]]))
