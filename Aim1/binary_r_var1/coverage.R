...
for (k in 1:2000) {
  CI_part <- get(load(paste("~/Aim1/binary_r_var1/CI_binary_500_rare_",k,".rda", sep = "")))
  for(z in 1:5){
    CI_full[[z]][[k]] <- CI_part[[z]][[k]] 
  }
}

save(CI_full, file = paste("CI_binary_500_rare.rda", sep = ""))

CI <- get(load("~/Aim1/binary_r_var1/CI_binary_500_rare.rda"))
