...
k = 1
    ...
    for(i in 1:nm){
      if(i == 1){
        ...
        fit2 <- zeroinfl(outcome[i,index1] ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1] | factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1],
                         na.action=na.omit, method = "BFGS", link = "log", EM = TRUE,
        ...
        start.coeff1 <- logbin(factor(outcome[i,index1]==0) ~ outcome[i,index1] ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1] + outcome[i-1,index1],
                               # family = binomial(link = "log"),
  ...
  save(CI, file = paste("CI_count_zibfgs_500_",k,".rda", sep = ""))
