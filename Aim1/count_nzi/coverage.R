...
save(CI_full, file = paste("CI_count_nzi_500_full.rda", sep = ""))

CI <- get(load("~/Aim1/count_nzi/CI_count_nzi_500_full.rda"))
...
save(CI_cov, file = paste("CIcoverage_count_nzi_500.rda", sep = ""))
...
var_names <- c("α0", "α1(t)", "α2", "α3'", "α4", 
               ...
               "α1(t)×β1(t)", "POF IE11", "POF IE01", "POF IE10", "POF IE00")

colnames(cov_perc) <- var_names
cov_perc_select <- cov_perc[,c(7,12:16)]

write.csv(cov_perc_select, paste("coverage_countnzi.csv"), row.names=FALSE)
...
id_med = 862
### Sample ID 862 rendered a product of coefficients estimate closest to the median ###
...
dat_IE11 <- as.data.frame(cbind(effect_interest[[3]][id_med,], CI[[13]][[id_med]]))
dat_IE01 <- as.data.frame(cbind(effect_interest[[4]][id_med,], CI[[14]][[id_med]]))
dat_IE10 <- as.data.frame(cbind(effect_interest[[5]][id_med,], CI[[15]][[id_med]]))
dat_IE00 <- as.data.frame(cbind(effect_interest[[6]][id_med,], CI[[16]][[id_med]]))
