...
for (k in 1:2000) {
  CI_part <- get(load(paste("~/Aim1/binary_c500/CI_binaryc_500_",k,".rda", sep = "")))
  for(z in 1:16){
...
CI <- get(load("~/Aim1/binary_c500/CI_binary_500_common.rda"))
...
write.csv(cov_perc_select, paste("coverage_binc.csv"), row.names=FALSE)
...
id_med = 384
### Sample ID 384 rendered a product of coefficients estimate closest to the median ###
