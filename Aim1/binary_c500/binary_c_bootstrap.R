...
results <- get(load("~/Aim1/binary_c500/results_binary_500_modelc.rda"))
...
# k = 1
  ...
  for (r in 1:R) {
    ...
    outcome <- LongToWide(coeff_data$id, coeff_data$time_pt, coeff_data$Yc)
    ...
    for(i in 1:nm){
      if(i == 1){
        fit2 <- logbin(factor(outcome[i,index1]) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1], 
                       start = c(log(start.p[i]), rep(c(0), 4)),
                       method = "em", maxit = 25000,
                       na.action=na.omit)
      } else {
        fit2 <- logbin(factor(outcome[i,index1]) ~ factor(trt[index1]) + factor(mediator[i,index1]) + factor(tvcon[i,index1]) + x1[index1] + factor(outcome[i-1,index1]), 
                       start = c(log(start.p[i]), rep(c(0), 5)),
                       method = "em", maxit = 25000,
                       na.action=na.omit)
  ...
  save(CI, file = paste("CI_binaryc_500_",k,".rda", sep = ""))
...
# save(CI, file = paste("CI_binary_500_common.rda", sep = ""))
