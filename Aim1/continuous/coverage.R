...
var_names <- c("α0", "α1(t)", "α2", "α3'", "α4", 
               ...
               "POC IE", "POF IE11", "POF IE01", "POF IE10", "POF IE00")

colnames(cov_perc) <- var_names
cov_perc_select <- cov_perc[,c(7,12:16)]

write.csv(cov_perc_select, paste("coverage_cont.csv"), row.names=FALSE)
...
id_med = 727
### Sample ID 727 rendered a product of coefficients estimate closest to the median ###
...
ggplot(data = dat_g, aes(t.est, V1)) +
  ...
  geom_line(aes(t.est, 0), linewidth = 0.7, color = "black") +
